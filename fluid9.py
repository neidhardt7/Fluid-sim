...
cohesive_force = 3.0    # Cohesive force parameter   # Suggested: 0.0, 0.5, 1.0
...
rho_wall = 0.5     # Wall density (normalized)   # Suggested: 0.5, 1.0, 2.0
...
droplet_center_y = droplet_radius  # Droplet center y-coordinate (so that it touches y=0)
...
# Initialize order parameter phi with a hyperbolic tangent profile.
## (Note: tanh profile smoothly connects phi ~ +1 inside the droplet to phi ~ -1 outside.)
...
# (Note: tanh profile smoothly connects phi ~ +1 inside the droplet to phi ~ -1 outside.)
# Scale the order parameter based on liquid and gas densities
density_scale = (rho_liquid - rho_gas) / (rho_liquid + rho_gas)  # Scale factor for density difference
density_offset = (rho_liquid + rho_gas) / 2.0  # Offset to center the density range

distance_from_center = np.sqrt((x_mesh - droplet_center_x)**2 + (y_mesh - droplet_center_y)**2)
# Scale the tanh profile to match the desired density range
order_parameter = density_scale * np.tanh((droplet_radius - distance_from_center) / np.sqrt(2)) + density_offset
...
for iteration in range(max_iterations):
    ...
    # Added cohesive force term: cohesive_force * (local_avg - order_parameter)
    ...
    if iteration % frame_interval == 0:
        animation_frames.append(np.copy(order_parameter))
